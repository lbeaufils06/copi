        ...
        {serverOffline ? (
          ...
          jobs.map((job) => {
            ...
            return (
              <div
                key={job.id}
                className="bg-slate-900 rounded-2xl p-4 sm:p-6 border border-slate-700"
              >
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                  <div className="space-y-2 text-sm">
                    <div className="flex flex-wrap items-center gap-2 sm:gap-3 md:gap-4">
                      <h3 className="text-base font-semibold px-2 py-1 rounded-md bg-slate-800/70 text-slate-100 inline-flex items-center max-w-full break-all">
                        {job.name}
                      </h3>
                      <span className="text-xs px-2 py-2 rounded-md bg-blue-900/40 text-blue-300 inline-flex items-center">
                        {job.dbType}
                      </span>
                      <span
                        className={`text-xs px-2 py-2 rounded-md inline-flex items-center max-w-full sm:max-w-[28rem] break-words bg-blue-900/40 text-blue-300`}
                      >
                    ...
                    <div className="flex flex-col sm:flex-col gap-2 sm:gap-2 w-full lg:w-auto text-sm pt-2">
                          <p>
                    ...
                    <div className="flex items-center gap-4">
                      <span className={`inline-flex items-center gap-2 text-sm px-3 py-2 rounded-lg font-medium ${statusStyle(job.lastStatus)}`}>
                          {isRunning && (
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      >
                        <span className="icon-tooltip-text hidden lg:block">{t("jobs.edit")}</span>
                        <svg
                      ...
                      >
                        <span className="icon-tooltip-text hidden lg:block">{t("jobs.history")}</span>
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
